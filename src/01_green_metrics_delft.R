...
if (file.exists(base_output)) {
  message("Loading existing Delft green metrics...")
  grid_metrics <- st_read(base_output, quiet = TRUE)
} else {
  message("Calculating Delft green metrics...")
  ...
  for (i in seq_len(nrow(grid))) {
    message("Delft green metrics: ", i, " / ", nrow(grid))
...
if (file.exists(heavy_rain_path)) {
  message("Adding Delft heavy rainfall layer...")

  heavy_rain <- rast(heavy_rain_path)
  heavy_rain <- project(heavy_rain, vect(grid))

  rain_mean <- terra::extract(
    heavy_rain,
    vect(grid),
    fun = mean,
    na.rm = TRUE
...
if (file.exists(dem_path)) {
  message("Adding Delft DEM variables...")
...
write.csv(
  ...
  "data/results/delft_grid_metrics.csv",
  row.names = FALSE
)

message("Delft script complete.")
